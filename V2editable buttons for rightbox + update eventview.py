...
def edit_event_with_fetch(event_name, event_date):
    ...
    if event_data:
        edit_event(event_name, event_date, event_data[0], event_data[1], event_data[2]) # Pass event_data
    else:
...
def edit_event(event_name, event_date, event_description=None, event_start_time=None, event_end_time=None):
    def update_event():
        # ... (Get values from edit_window entries) 
    ...
    # Add labels and entry fields for editable attributes
    tk.Label(edit_window, text="Event Name:").grid(row=0, column=0)
    edit_name_entry = tk.Entry(edit_window)
    edit_name_entry.insert(0, event_name) 
    edit_name_entry.grid(row=0, column=1)

    tk.Label(edit_window, text="Event Date (DD.MM.YYYY):").grid(row=1, column=0)
    edit_date_entry = tk.Entry(edit_window)
    edit_date_entry.insert(0, event_date)  
    edit_date_entry.grid(row=1, column=1)

    tk.Label(edit_window, text="Event Description:").grid(row=2, column=0)
    edit_description_entry = tk.Entry(edit_window)
    edit_description_entry.insert(0, event_description)  
    edit_description_entry.grid(row=2, column=1)
    
    tk.Label(edit_window, text="Start Time (HH:MM):").grid(row=3, column=0)
    edit_start_time_entry = tk.Entry(edit_window)
    edit_start_time_entry.insert(0, event_start_time or "") 
    edit_start_time_entry.grid(row=3, column=1)

    tk.Label(edit_window, text="End Time (HH:MM):").grid(row=4, column=0)
    edit_end_time_entry = tk.Entry(edit_window)
    edit_end_time_entry.insert(0, event_end_time or "") 
    edit_end_time_entry.grid(row=4, column=1)


    # Add the Update button
    update_button = tk.Button(edit_window, text="Update", command=update_event)
    update_button.pack()
